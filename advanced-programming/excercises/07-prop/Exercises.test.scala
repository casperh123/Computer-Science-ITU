...
object PropSpec
  ...
  property("Ex13.01: && two successes are a success") =
    sc.Prop.forAll { (g: Gen[Int], rng: RNG) =>
      ! (ex13T(g) && ex13T(g)) (MaxSize(-1), TestCases(10), rng).isFalsified }

  property("Ex13.02: && both failures are failure") =
    sc.Prop.forAll { (g: Gen[Int], rng: RNG) =>
      (ex13F(g) && ex13F(g))(MaxSize(-1), TestCases(10), rng).isFalsified }

  property("Ex13.03: && left failure is failure") =
    sc.Prop.forAll { (g: Gen[Int], rng: RNG) =>
      (ex13F(g) && ex13T(g))(MaxSize(-1), TestCases(10), rng).isFalsified }

  property("Ex13.04: && right failure is failure") =
    sc.Prop.forAll { (g: Gen[Int], rng: RNG) =>
      (ex13T(g) && ex13F(g))(MaxSize(-1), TestCases(10), rng).isFalsified }
