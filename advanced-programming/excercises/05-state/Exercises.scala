...
object RNG:
  ...
  def nonNegativeInt(rng: RNG): (Int, RNG) =
    ...
    n match
      case randomInt if randomInt < 0 => nonNegativeInt(newRng)
      case randomInt => (randomInt, newRng)
  ...
  def intDouble(rng: RNG): ((Int, Double), RNG) =
    val (randomInt, intRng) = nonNegativeInt(rng)
    val (randomDouble, newRng) = double(intRng)
    ((randomInt, randomDouble), newRng)


  // The return type is broken and needs to be fixed
  def doubleInt(rng: RNG): ((Double, Int), RNG) =
    val ((randomInt, randomDouble), newRng) = intDouble(rng)
    ((randomDouble, randomInt), newRng)

  // Exercise 4

  // The return type is broken and needs to be fixed
  def ints(size: Int)(rng: RNG): (List[Int], RNG) = size match
    case 0 =>
      (List.empty[Int], rng)
    case _ =>
      val (randomInt, newRng) = rng.nextInt
      val (nextRandomInt, finalRng) = ints(size - 1)(newRng)
      (randomInt::nextRandomInt, finalRng)
  ...
  lazy val double2: Rand[Double] =
    map(nonNegativeInt)((randomInt) => randomInt.toDouble / Int.MaxValue)
  ...
  def sequence[A](ras: List[Rand[A]]): Rand[List[A]] =
    ras.foldRight(unit(List.empty[A]))((rand, acc) =>
      map2(rand, acc)((first, second) => first::second)
    )
...
case class State[S, +A](run: S => (A, S)):
  ...
  def map[B](f: A => B): State[S, B] =
    State(s =>
      val (value, nextState) = this.run(s)
      (f(value), nextState)
    )

  def map2[B,C](sb: State[S, B])(f: (A, B) => C): State[S, C] =
    State(s =>
      val (a, aState) = this.run(s)
      val (b, bState) = sb.run(aState)
      (f(a, b), bState)
    )
