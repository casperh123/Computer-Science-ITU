...
object MonoidSpec extends org.scalacheck.Properties("monoid"):

  // Pre Exercises. Monoid tests for strings and lists

  property("Ex04.01: stringMonoid is a monoid") = stringMonoid.laws.monoid
  ...
  property("Ex05.01: foldMap a constant list with identity") = forAll {
    (l: List[Int], n: Int) =>
      intAddition.foldMap(l.map { _ => n })(identity[Int]) == l.size * n
  }

  property("Ex05.02: foldMap a constant list with incr") = forAll {
    (l: List[Int], n: Int) =>
...
object FoldableSpec extends org.scalacheck.Properties("foldab"):
  ...
  property("Ex12.02: toListF on intFoldable gives List.fill(n)(a)") = forAll {
    (n: Int, a: Double) =>
      val n1 = (n % 100).abs
...
object FunctorSpec extends org.scalacheck.Properties("functo"):
...
object MonadSpec extends org.scalacheck.Properties("monad_"):
  ...
  given Monad[Id] = new:
    ...
    extension [A](a: Id[A]) def flatMap[B](f: A => Id[B]): Id[B] = f(a)

  property("Ex17.01: sequence in idMonad (1)") = forAll { (l: List[Int]) =>
    summon[Monad[Id]].sequence[Int](l) == l
  }

  property("Ex17.02: sequence in idMonad (2)") = forAll { (l: List[Unit]) =>
    summon[Monad[Id]].sequence[Unit](l) == l
  }

  property("Ex17.03: sequence in optionMonad (1)") = forAll { (l: List[Int]) =>
    val oas = l.map { Some(_) }
    optionMonad.sequence(oas) == Some(l)
  }

  property("Ex17.04: sequence in optionMonad (2)") = forAll { (n: Int) =>
  ...
  property("Ex18.01: replicateM in Id") = forAll(Gen.choose(0, 100)) {
    (n: Int) =>
      summon[Monad[Id]].replicateM(n, n) == List.fill(n)(n)
  }

  property("Ex18.02: replicateM in Option") = forAll(Gen.choose(0, 100)) {
    (n: Int) =>
  ...
  property("Ex19.01: Kleisli composition in Id") = forAll {
    (f: Int => Id[Int], g: Int => Id[String], x: Int) =>
      summon[Monad[Id]].compose[Int, Int, String](f, g)(x) == g(f(x))
  }

  property("Ex19.02: Kleisli composition in Option") = forAll {
    (f: Int => Int, g: Int => String, x: Int) =>
