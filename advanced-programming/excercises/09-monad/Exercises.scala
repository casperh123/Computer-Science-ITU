...
def optionMonoidLift[A: Monoid]: Monoid[Option[A]] = new Monoid[Option[A]]:
  def combine(a: Option[A], b: Option[A]): Option[A] =
    val combined = for
      x <- a
      y <- b
    yield summon[Monoid[A]].combine(x, y)
...
def endoMonoid[A]: Monoid[A => A] = new Monoid[A => A]:
  def combine(a: A => A, b: A => A): A => A = parameter => a(b(parameter))
  val empty = identity;
...
extension [B](mb: Monoid[B])
  def foldMap[A](as: List[A])(f: A => B): B =
    as.foldRight(mb.empty)((item, acc) => mb.combine(f(item), acc))
...
extension [A: {Arbitrary, Equality}](ma: Monoid[A])
  ...
  def homomorphism[B](f: A => B)(mb: Monoid[B]): Prop =
    forAll{(first: A, second: A) =>
      mb.combine(f(first), f(second)) === f(ma.combine(first, second))
    }

  def isomorphism[B: Arbitrary](f: A => B, g: B => A)(mb: Monoid[B]): Prop = {
    ma.homomorphism(f)(mb) &&
    mb.homomorphism(g)(ma) &&
    forAll{(a: A) =>
      g(f(a)) === a
    } &&
    forAll{(b: B) =>
...
object MonoidEx7Spec
  ...
  property("Ex07.01: stringMonoid is isomorphic to listMonoid[Char]") =
    stringMonoid.isomorphism((a: String) => a.toList, (b: List[Char]) => b.mkString)(listMonoid[Char])
...
object MonoidEx8Spec
  ...
  property("Ex08.01: booleanOr is isomorphic to booleanAnd") =
    booleanOr.isomorphism((a: Boolean) => !a, (b: Boolean) => !b)(booleanAnd)


// Exercise 9

def productMonoid[A, B](ma: Monoid[A])(mb: Monoid[B]): Monoid[(A, B)] = new Monoid[(A, B)]:
  def combine(first: (A, B), second: (A,B)): (A, B) = (ma.combine(first._1, second._1), mb.combine(first._2, second._2))
...
object MonoidEx10Spec
  ...
  property("Ex10.01: productMonoid(optionMonoid[Int])(listMonoid[String]) gives a monoid") =
    val sut = productMonoid(optionMonoid[Int])(listMonoid[String])
    sut.laws.monoid
    sut.laws.associative
    sut.laws.unit
...
given foldableList: [A] => Foldable[List] = new Foldable[List]:
  extension [A](as: List[A])
    def foldMap[B](f: A => B)(using mb: Monoid[B]): B = as.map(f).foldRight(mb.empty)((item, acc) => mb.combine(item, acc))
...
extension [F[_]: Foldable, A] (as: F[A])
  ...
  def toListF: List[A] =
    as.foldMap(a => List(a))(using listMonoid[A])
...
object FunctorEx16Spec
  ...
  property("Ex16.01: optionMonad is a monad") =
    optionMonad.monadLaws.monad[Int, String, Boolean]

  property("Ex16.02: listMonad is a monad") =
    listMonad.monadLaws.monad[Int, String, Boolean]
...
extension [F[_]](m: Monad[F])
  def sequence[A](fas: List[F[A]]): F[List[A]] =
    fas.foldRight(m.unit(List.empty[A]))((fa, acc) => m.map2(fa)(acc)((a, list) => a::list))
