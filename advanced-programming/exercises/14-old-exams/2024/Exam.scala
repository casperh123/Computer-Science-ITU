...
object Good:
  ...
  def goodPairs [A] (l: List[A], good: (A,A) => Boolean): Boolean =
    l.sliding(2).forall(window => good(window(0), window(1)))
  ...
  def curriedNested [A, B, C, D] (f: ((A,B) => C) => D)
    : (A => B => C) => D = 
    curriedArg => f((a, b) => curriedArg(a)(b))
  ...
  list => curriedNested(goodPairs[A](list, _))
...
object Gens:
  ...
  def genEither[A,B](using Arbitrary[Option[A]], Arbitrary[Option[B]]): Gen[Either[A,B]] = 
    for
      optionA <- arbitrary[Option[A]]
      optionB <- arbitrary[Option[B]]
      result <- optionA.flatMap(a =>
        optionB.map {b => 
          Gen.oneOf(Left(a), Right(b))  
        }
  ...
  // ...
    ...
    extension[P[+_]] (p: Parsers[ParseError, P])
      ...
      def intBetween(low: Int, high: Int): P[Option[Int]] = 
        import p.*
        INT.map(s => if low < s && low < high then Some(s) else None)
...
object FullyAbstractTrains:
  ...
  trait ReservationSystem[
    ...
    Amount: Arbitrary, Passenger: Arbitrary, TrainNo: Arbitrary, PaymentId: Arbitrary, TicketNo,
    CC: Arbitrary, Err
  ]:
      ...
      // Create a reservation, returns a ticket number if successful, or an error
      def reserve (passenger: Passenger, train: TrainNo, paymentId: PaymentId)
        : Either[Err, TicketNo]
      ...
      // Returns a set of passengers on the train (a manifest)
      def paxOnTrain (train: TrainNo): QueryResult[Passenger]
     ...
     **/
      ...
      def law1: Prop =
        forAll { (creditCard: CC, amount: Amount) =>
          pay(creditCard, amount).map (validate).isRight
        }
      ...
      */
      ...
      def law2: Prop = 
        forAll {(passenger: Passenger, train: TrainNo, paymentId: PaymentId) =>
          reserve(passenger, train, paymentId)
            .map(reservationId =>
              summon[Member[QueryResult]].contains(paxOnTrain(train), reservationId)
            ).getOrElse(true)
        }
