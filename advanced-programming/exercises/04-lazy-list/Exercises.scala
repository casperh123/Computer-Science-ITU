...
enum LazyList[+A]:
  ...
  this.foldRight(Empty: LazyList[A])((elem, acc) => 
    if p(elem) then cons(elem, acc) else Empty)

  // Exercise 7
  
  def headOption1: Option[A] = 
    this.foldRight(None)((elem, acc) => Some(elem))
  ...
  def takeUnfold(n: Int): LazyList[A] =
    unfold((this, n))((list, index) => 
      if n > 0
      then list.headOption.map(head => (head, (list.tail, index - 1)))
      else None
    )

  def takeWhileUnfold(p: A => Boolean): LazyList[A] =
    unfold(this)(list => list.headOption.flatMap(head =>
      if p(head)
...
object LazyList:
  ...
  lazy val fibs: LazyList[Int] = 
    def inner(first: Int, second: Int): LazyList[Int] =
      Cons(() => first, () => inner(second, first + second))
