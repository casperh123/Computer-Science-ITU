...
object LazyListSpec 
  ...
  def exceptionList: Gen[List[RuntimeException]] =
    Gen.sized { s => Gen.listOfN(s max 1, Gen.const(new RuntimeException("tail was forced"))) }
  ...
  property("Ex02.01: headOption does not force tail") =
    given Arbitrary[List[RuntimeException]] = Arbitrary(exceptionList)
    
    forAll { (tail: List[RuntimeException]) =>
      (1::tail).headOption == Some(1)
    }
  ...
  property("Ex03.01: take does not force any heads nor any tails") =
    ...
    forAll { list: List[RuntimeException] => 
