...
extension (self: Prop)
  infix def && (that: Prop): Prop = (maxSize, tcs, rng) =>
    self.apply(maxSize, tcs, rng) match
      case Passed => that.apply(maxSize, tcs, rng)
      case Falsified(sucesses, failures) => Falsified(sucesses, failures)
    

  infix def || (that: Prop): Prop = (maxSize, tcs, rng) =>
    self.apply(maxSize, tcs, rng) match
      case Falsified(failure, successes) => that.apply(maxSize, tcs, rng)
      case Passed => Passed
