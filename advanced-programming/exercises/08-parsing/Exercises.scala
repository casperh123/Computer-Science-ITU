...
class JSONParser[ParseError, Parser[+_]](P: Parsers[ParseError,Parser]):
  ...
  lazy val jarray: Parser[JArray] =
    val values = commaSeparatedVals | succeed(List.empty)

    (char('[') |* ws |* (values) *| ws *| char(']')).map(array => JArray(array.toIndexedSeq))

  lazy val field: Parser[(String,JSON)] =
    (QUOTED *| ws) ** (char(':') |* ws |* json)

  private lazy val commaSeparatedFields: Parser[List[(String,JSON)]] =
    val fields = (char(',') |* ws |* field *| ws).many
  ...
  lazy val jobject: Parser[JObject] =
    val fieldsOrEmpty = commaSeparatedFields | succeed(List.empty)
    val fields = char('{') |* ws |* fieldsOrEmpty *| ws *| char('}')

    fields.map(fs => JObject(fs.toMap))
