error id: file://<WORKSPACE>/Exercises.scala:local115
file://<WORKSPACE>/Exercises.scala
empty definition using pc, found symbol in pc: 
found definition using semanticdb; symbol local115
...
class JSONParser[ParseError, Parser[+_]](P: Parsers[ParseError,Parser]):
  ...
  lazy val jarray: Parser[JArray] =
    for 
      first <- string("[")
      whitespace <- ws
      array <- commaSeparatedVals | succeed(List.empty)
      whitespace <- ws
      second <- string("]")
    yield JArray(array.toIndexedSeq)
  ...
  lazy val jobject: Parser[JObject] =
    ...
    yield JObject(Map(string.get -> @@value))
