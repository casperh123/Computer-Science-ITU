error id: 0F185120830A604C2B960E0A7FA6C63C
file://<WORKSPACE>/Exercises.scala
### dotty.tools.dotc.ast.Trees$UnAssignedTypeException: type of Ident(Parser) is not assigned
...
trait Parsers[ParseError, Parser[+_]]:
  ...
  extension [A](p: Parser[A]) 
    def many: Parser[List[A]] = 
      p.map2(p.many)(_::_) | succeed(List.empty)
  ...
  def digitTimesA: Parser[Int] =
    regex("[0-9]".r).flatMap { stringDigit => 
      val digit = stringDigit.toInt
      char('a').listOfN(digit).map( _ => digit)
    }
...
class JSONParser[ParseError, Parser[+_]](P: Parsers[ParseError,Parser]):
  ...
  lazy val QUOTED: Parser[String] =
    string("\"").flatMap(quote => regex("[^\"]+".r).flatMap(content => string("\"").map(second => content)))

  lazy val DOUBLE: Parser[Double] =
    regex("[-+]?[0-9]*\\.?[0-9]+([eE][-+]?[0-9]+)?".r).map(_.toDouble)

  lazy val ws: Parser[Unit] =
    regex("\\s+".r).map(_ => ()) | succeed(())

  // Exercise 13
  
  lazy val jnull: Parser[JSON] =
    string("null").map(value => JNull)

  lazy val jbool: Parser[JSON] =
    (string("true") | string("false")).map(value => JBool(value.toBoolean))

  lazy val jstring: Parser[JString] =
    QUOTED.map(value => JString(value))

  lazy val jnumber: Parser[JNumber] =
    DOUBLE.map(value => JNumber(value))

  // Exercise 13

  private lazy val jvalue: Parser[JSON] =
