error id: file://<WORKSPACE>/Exercises.scala:local109
file://<WORKSPACE>/Exercises.scala
empty definition using pc, found symbol in pc: 
found definition using semanticdb; symbol local109
...
class JSONParser[ParseError, Parser[+_]](P: Parsers[ParseError,Parser]):
  ...
  private lazy val commaSeparatedVals: Parser[List[JSON]] =
    s => match 
    for 
      arrayValue@@ <- (jstring.flatMap(value => char(',').map(comma => value))).many
    yield arrayValue
